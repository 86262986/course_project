...
if __name__ == "__main__":
    ...
    print('Leaking k = {}'.format(hex(k_leak)))
    
    e = hash_message(msg)
    r,s = signature
    d_guess = (inverse_mod(r, curve.n)*(k_leak*s - e))%curve.n
    print('guess private key d = {0}'.format(hex(d_guess)))

    if d_guess == d:print('Success!')
    else:print('Failed.')

    print('\n(2) Reusing k leads to leaking of d:')
    print('Reusing k = {}'.format(hex(k_leak)))
    ...
    print('guess private key d = {0}'.format(hex(d_guess2)))
    ...
    print("Alice's private key d1:", hex(d1))
    print("Alice's public key P1: (0x{:x}, 0x{:x})".format(*P1))
    print("Alice's message:",m1)
    print("\nBob's private key d2:", hex(d2))
    print("Bob's public key P2: (0x{:x}, 0x{:x})".format(*P2))
    print("Bob's message:",m2)
    print()

    d_guess_Bob = ((s_2*e_1-s_1*e_2+s_2*r_1*d1)*inverse_mod(s_1*r_1,curve.n)) % curve.n
    print("Alice recovered Bob's private key:",hex(d_guess_Bob))
    if d_guess_Bob == d2:print('Success')
    else:print('Failed')

    d_guess_Alice = ((s_1*e_2-s_2*e_1+s_1*r_1*d2)*inverse_mod(s_2*r_1,curve.n)) % curve.n
    print("Bob recovered Alice's private key:",hex(d_guess_Alice))
